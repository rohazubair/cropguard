...
def crop_envelopes(df: pd.DataFrame) -> dict[tuple[int, str], dict[str, float]]:
    ...
    for (did, crop), g in sub.groupby(['district_id', 'crop_name'], sort=False):
        out[int(did), str(crop)] = {'t_p10': float(g['temperature'].quantile(0.1)), 't_p50': float(g['temperature'].quantile(0.5)), 't_p90': float(g['temperature'].quantile(0.9)), 'h_p10': float(g['humidity'].quantile(0.1)), 'h_p50': float(g['humidity'].quantile(0.5)), 'h_p90': float(g['humidity'].quantile(0.9))}
    return out

def score_crop_day(t_pred: float, h_pred: float, env: dict[str, float]) -> tuple[float, str, dict[str, Any]]:
    drivers: dict[str, Any] = {}
    score = 0.0
    t90 = max(env['t_p90'], 1e-06)
    t10 = max(env['t_p10'], 1e-06)
    h90 = max(env['h_p90'], 1e-06)
    h10 = max(env['h_p10'], 1e-06)
    if t_pred > env['t_p90']:
