...
from prediction.forecast_db import (
    db_conn,
    finish_run,
    get_active_model,
    insert_crop_risk_points,
    insert_model_registry,
    insert_run,
    insert_weather_points,
    publish_run,
    utc_now_naive,
)
...
def run_forecast_pipeline(*, project_root: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        need_days = LOOKBACK_DAYS + HORIZON_DAYS + 3
        ...
        if retrain:
            try:
                ...
                insert_model_registry(
                    ...
                    version=version,
                    artifact_uri=uri,
                    train_data_end_ts=train_end_ts,
                    metrics=metrics,
        ...
        run_id = insert_run(
            cur,
            model_id=int(active2["model_id"]),
            input_weather_until=input_until_naive,
        ...
        finish_run(
            ...
            status="success",
            drift_checked=True,
            drift_retrain_triggered=summary.get("retrained", False),
            notes=None,
        )
        ...
        summary.update(
            {
                ...
                "weather_points": len(weather_rows),
                "crop_risk_points": len(risk_rows),
                "model_id": int(active2["model_id"]),
            }
        )
        return summary
