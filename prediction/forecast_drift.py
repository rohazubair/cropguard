...
def should_retrain_from_drift(daily: pd.DataFrame, drift_reference: dict[str, Any] | None, *, recent_days: int=7, threshold: float=DRIFT_Z_THRESHOLD) -> bool:
    ...
    for did, mu_r in recent.items():
        ...
        sig = float(max(1e-06, float(cell.get('std', 1e-06))))
