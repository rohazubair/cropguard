...
from prediction.forecast_constants import ARTIFACT_SUBDIR, HORIZON_DAYS, LOOKBACK_DAYS, MIN_TRAIN_ROWS, MODEL_NAME_WEATHER_DAILY
from prediction.forecast_daily_dataset import build_supervised_daily, drift_reference_from_daily, hourly_to_daily

def _make_pipeline(quantile: float) -> Pipeline:
    return Pipeline(steps=[('prep', ColumnTransformer(transformers=[('cat', OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1), ['district_id'])], remainder='passthrough')), ('model', MultiOutputRegressor(HistGradientBoostingRegressor(loss='quantile', quantile=quantile, learning_rate=0.07, max_depth=12, max_iter=280, l2_regularization=0.2, min_samples_leaf=10, random_state=42)))])
...
def fit_weather_bundle(daily: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    X_tr, X_va, yt_tr, yt_va, yh_tr, yh_va = train_test_split(X, y_temp, y_hum, test_size=0.15, random_state=42, shuffle=True)
    ...
    meta = {'model_name': MODEL_NAME_WEATHER_DAILY, 'n_supervised_rows': int(len(sup)), 'n_daily_rows': int(len(daily)), 'train_data_end_date': str(train_end.date()), 'val_mae_temp_p50': val_mae_temp.get('0.5'), 'val_mae_humidity_p50': val_mae_hum.get('0.5'), 'feature_cols': feature_cols, 'horizon_days': HORIZON_DAYS, 'lookback_days': LOOKBACK_DAYS}
    bundle: dict[str, Any] = {'pipelines_temp': pipes_temp, 'pipelines_humidity': pipes_hum, 'feature_cols': feature_cols, 'drift_reference': drift_reference, 'meta': meta}
    metrics = {'val_mae_temp': val_mae_temp, 'val_mae_humidity': val_mae_hum, 'n_train_rows': int(len(X))}
