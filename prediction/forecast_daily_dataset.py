...
def hourly_to_daily(hourly: pd.DataFrame) -> pd.DataFrame:
    ...
    g = df.groupby(['district_id', 'date'], as_index=False).agg(temp_max=('temperature', 'max'), humidity_mean=('humidity_pct', 'mean')).sort_values(['district_id', 'date']).reset_index(drop=True)
    return g
...
def drift_reference_from_daily(daily: pd.DataFrame) -> dict[str, Any]:
    ...
    for did, g in daily.groupby('district_id', sort=False):
        per[str(int(did))] = {'mean': float(g['temp_max'].mean()), 'std': float(max(1e-06, float(g['temp_max'].std(ddof=0) or 0.0)))}
