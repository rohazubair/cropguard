...
def _get_json(api_base: str, path: str) -> dict[str, Any]:
    ...
    try:
        ...
        st.error(f'Could not reach CropGuard API at `{url}`. Start the server with `uvicorn api.app:app --reload` from the repo root. ({exc})')
        st.stop()
    return r.json()
...
def main() -> None:
    ...
    with st.sidebar:
        st.header('About')
        st.markdown('This app loads the **published** ML forecast run through the CropGuard API. Run **`uvicorn api.app:app`** from the repo root, then refresh here. Populate data with `python main.py` or `python main.py --forecast-only`.')
    ...
    if run_df.empty and weather_df.empty and risk_df.empty:
        st.warning('No published forecast data yet. The gold views are empty until a forecast run completes successfully.')
        st.stop()
    ...
    with tab_risk:
        ...
        if risk_df.empty:
            ...
            st.dataframe(filt.drop(columns=['_tier_ord'], errors='ignore'), use_container_width=True, hide_index=True)
