"""Read-only HTTP access to `gold.v_dashboard_*` for the Streamlit app."""
...
from ingestion.ingest_weather import connect_db  # noqa: E402
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            try:
                ...
                if getattr(exc, "pgcode", None) == errorcodes.UNDEFINED_TABLE:
                    raise HTTPException(
                        status_code=503,
                        detail="Gold dashboard views are missing. Apply `storage/setup_db.sql` and run the forecast pipeline.",
                    ) from exc
