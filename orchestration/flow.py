...
try:
    ...
    try:
        from prefect import exponential_backoff  # type: ignore[attr-defined,no-redef]
    except ImportError:
        exponential_backoff = None  # type: ignore[misc, assignment]
...
# Three retries after the first attempt; delay grows ~exponentially between retries.
_RETRIES = 3
if exponential_backoff is not None:
    _RETRY_DELAY = exponential_backoff(backoff_factor=8)
else:
    # Prefect 2: list length must equal `retries` (delay before each retry) — exponential steps in seconds.
...
def task_ingest_district_weather(district: dict) -> str:
    ...
    return (
        f"Success: {name} "
        f"(insert_attempts={summary.get('hours_insert_attempted', 0)}, "
        f"cdc_skipped_hours={summary.get('hours_skipped_cdc', 0)})"
    )
...
def cropguard_bronze_pipeline() -> dict[str, Any]:
    """
    DDL → district geocode → crop CSV → silver crop_facts → Open-Meteo hourly per district → silver weather
    → forecast (drift-gated retrain + infer + publish).
    Each step runs as a Prefect task with 3 retries and exponential backoff between retries.
    """
    log = get_run_logger()
    ...
    if not districts:
        log.warning(
            "No rows in bronze.districts_dim; skipping per-district weather ingest"
        )
        silver_weather_summary = task_load_weather_readings_silver()
        forecast_summary = task_forecast_pipeline()
        log.info(
            "Bronze pipeline finished (no districts; forecast_keys=%s)",
            list(forecast_summary.keys()) if isinstance(forecast_summary, dict) else type(forecast_summary).__name__,
        )
        return {
            "districts": [],
            "seed_districts": seed_summary,
            "crop_facts": crop_summary,
            "silver_crop_facts": silver_crop_summary,
            "silver_weather_readings": silver_weather_summary,
    ...
    forecast_summary = task_forecast_pipeline()
    log.info(
        "Bronze pipeline finished (districts=%s, forecast_keys=%s)",
        len(districts),
        list(forecast_summary.keys()) if isinstance(forecast_summary, dict) else type(forecast_summary).__name__,
    )
    return {
...
def weather_pipeline():
    """Backward-compatible name: runs the full bronze Prefect pipeline."""
