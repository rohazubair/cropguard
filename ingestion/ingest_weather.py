...
def _database_url():
    ...
    if not url:
        raise RuntimeError('DATABASE_URL is not set. Add it to .env in the cropguard folder or export it in your environment.')
    return url.strip().strip('"')

def _configure_search_path(cur):
    cur.execute(sql.SQL('SET search_path TO {}, public').format(sql.Identifier(BRONZE_SCHEMA)))
...
def get_districts():
    ...
    try:
        ...
        conn.close()
    return [{'district_id': row[0], 'name': row[1], 'lat': float(row[2]), 'lon': float(row[3])} for row in rows]

def fetch_weather(lat: float, lon: float, *, past_days: int=45, forecast_days: int=7, max_observed_ts: datetime | None=None) -> dict[str, Any]:
    params: dict[str, Any] = {'latitude': lat, 'longitude': lon, 'hourly': HOURLY_VARS, 'timezone': 'UTC'}
...
def save_to_db(district: dict[str, Any], *, past_days: int=45, forecast_days: int=7) -> dict[str, Any]:
    ...
    payload = fetch_weather(lat, lon, past_days=past_days, forecast_days=forecast_days, max_observed_ts=max_ts)
    ...
    if not times or not temps or (not humid):
        return {'district_id': district_id, 'hours_insert_attempted': 0, 'hours_in_response': 0, 'hours_skipped_cdc': 0, 'cdc_anchor_utc': max_ts.isoformat() if max_ts else None, 'note': 'missing hourly arrays in API response'}
    ...
    for i in range(n):
        ...
        rows.append((district_id, observed.replace(tzinfo=None), float(t_val), hum))
    conn = connect_db()
    try:
        cur = conn.cursor()
        _configure_search_path(cur)
        ...
        conn.close()
    return {'district_id': district_id, 'hours_insert_attempted': len(rows), 'hours_in_response': n, 'hours_skipped_cdc': skipped_cdc, 'cdc_anchor_utc': max_ts.isoformat() if max_ts else None}
