...
def _median_impute_by_district_then_global(series: pd.Series, district_id: pd.Series) -> tuple[pd.Series, int]:
...
def load_weather_readings_silver(*, truncate_before_load: bool=True) -> dict[str, Any]:
    ...
    try:
        ...
        if not df.empty:
            df['temperature'], imputed_temperature = _median_impute_by_district_then_global(df['temperature'], df['district_id'])
            h_float, imputed_humidity = _median_impute_by_district_then_global(df['humidity_pct'], df['district_id'])
        ...
        out_rows = list(zip(df['district_id'].astype(int), df['observed_ts'], df['temperature'].astype(float), df['humidity_pct'].astype(int))) if not df.empty else []
        ...
        conn.close()
    return {'bronze_rows_read': len(rows), 'rows_inserted': len(out_rows), 'rows_dropped_missing_keys': dropped_missing_keys, 'imputed_temperature_cells': imputed_temperature, 'imputed_humidity_cells': imputed_humidity, 'great_expectations': {'silver_weather_readings': ge_weather}}
