...
def _match_dot_initials_remainder(text: str) -> str | None:
    """
    Handles patterns like m.b.din, d.g.khan: single-letter segments then a trailing token.
    No literal district map — pattern match against canonical word lists.
    """
...
def _fuzzy_against_canonical(raw_c: str, raw_spaced: str, canonical: str, *, short_token: bool) -> float:
    ...
    parts = [
        _ratio(raw_c, cc),
        fuzz.ratio(raw_c, cc) / 100.0,
    ]
    if not short_token:
        parts.extend(
            [
                fuzz.partial_ratio(raw_c, cc) / 100.0,
                fuzz.WRatio(raw_c, canonical) / 100.0,
            ]
        )
    if raw_spaced:
...
def resolve_crop_district(raw: str | None) -> str | None:
    """
    Map a raw bronze/CSV district string to one of CANONICAL_CROP_DISTRICTS, or None if no confident match.
    Uses dot-initial patterns, compact equality, acronyms, unique prefixes, then fuzzy scores.
    """
    if raw is None:
        return None
    text = str(raw).strip()
    ...
    for name, _ in canon_compacts:
        scores.append(
            (name, _fuzzy_against_canonical(raw_c, raw_spaced, name, short_token=short_token)),
        )
