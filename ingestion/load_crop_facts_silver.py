...
SILVER_CROP_FIELD_PLAN: tuple[tuple[str, str], ...] = (
...
def _prepare_silver_frame(
    ...
    df = pd.DataFrame.from_records(
        records,
        columns=["district_id", "temperature", "humidity", "crop_name", "district"],
    )
    if df.empty:
        return df, {
            "dropped_unmapped": dropped_unmapped,
            "dropped_no_dim_id": dropped_no_dim_id,
    ...
    return df, {
        "dropped_unmapped": dropped_unmapped,
        "dropped_no_dim_id": dropped_no_dim_id,
        "dropped_empty_label": dropped_empty_label,
    }
...
def load_crop_facts_silver(*, truncate_before_load: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        "rows_dropped_unmapped": prep_stats.get("dropped_unmapped", 0),
        "rows_dropped_no_dim_id": prep_stats.get("dropped_no_dim_id", 0),
        "rows_dropped_empty_crop_name": prep_stats.get("dropped_empty_label", 0),
        "required_district_ids_count": len(required_ids),
        "canonical_districts_missing_from_dim": missing_canonical,
        "crops_before_full_coverage_filter": before_crop_count,
        "crops_after_full_coverage_filter": after_crop_count,
        "crops_removed_missing_districts": rows_dropped_crop_filter,
        "rows_removed_missing_district_coverage": rows_removed_coverage,
        "imputed_temperature_cells": imputed_temperature,
        "imputed_humidity_cells": imputed_humidity,
        "silver_fields": ["district_id"] + [s for _, s in SILVER_CROP_FIELD_PLAN] + ["district"],
        "great_expectations": {"silver_crop_facts": ge_crop},
    }
