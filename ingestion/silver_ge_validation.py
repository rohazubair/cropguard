...
_DISTRICT_ID_MAX = 2 ** 63 - 1

def _gx_validate(df: pd.DataFrame, *, suite_name: str, add_expectations: Callable[[ExpectationSuite], None]) -> dict[str, Any]:
    if df.empty:
    ...
    out: dict[str, Any] = {'success': bool(result.success), 'evaluated_expectations': stats.get('evaluated_expectations'), 'successful_expectations': stats.get('successful_expectations'), 'unsuccessful_expectations': stats.get('unsuccessful_expectations'), 'success_percent': stats.get('success_percent')}
    if not result.success:
        ...
        for r in getattr(result, 'results', []) or []:
            ...
            ex_msg = None
            if msg is not None:
                ex_msg = getattr(msg, 'exception_message', None) or (msg.get('exception_message') if isinstance(msg, dict) else None)
            failures.append(f"{typ}: {ex_msg or 'failed'}")
        out['failure_messages'] = failures[:20]
        raise ValueError('Great Expectations validation failed for silver dataframe: ' + '; '.join(failures[:8]))
    return out
...
def _add_crop_expectations(suite: ExpectationSuite) -> None:
    ...
    suite.add_expectation(E.ExpectColumnValuesToBeBetween(column='district_id', min_value=1, max_value=_DISTRICT_ID_MAX))
