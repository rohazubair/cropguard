...
def load_crop_facts_bronze(csv_path: Path | None=None, *, truncate_before_load: bool=True, skip_if_csv_unchanged: bool=True) -> dict[str, int | bool | str]:
    ...
    try:
        ...
        if skip_if_csv_unchanged:
            stored_row = None
            try:
                cur.execute('SELECT content_hash FROM bronze.ingestion_state WHERE key = %s', (CROP_CSV_STATE_KEY,))
            ...
            if stored_row and stored_row[0] == file_hash:
                cur.close()
                return {'skipped': True, 'reason': 'crop_csv_unchanged', 'content_hash': file_hash, 'rows_inserted': 0}
        ...
        rows = list(zip(df['N'].astype(int), df['P'].astype(int), df['K'].astype(int), df['temperature'].astype(float), df['humidity'].astype(float), df['ph'].astype(float), df['rainfall'].astype(float), df['label'], df['district']))
        ...
        conn.close()
    return {'skipped': False, 'content_hash': file_hash, 'rows_inserted': len(rows)}
if __name__ == '__main__':
    ...
    parser.add_argument('--no-truncate', action='store_true', help='Append without truncating (may duplicate rows)')
    parser.add_argument('--force', action='store_true', help='Reload even if CSV hash unchanged')
    args = parser.parse_args()
    path = Path(args.csv).resolve() if args.csv else None
    out = load_crop_facts_bronze(path, truncate_before_load=not args.no_truncate, skip_if_csv_unchanged=not args.force)
    print(out)
