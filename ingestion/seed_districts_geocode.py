"""Geocode district HQs via OpenStreetMap Nominatim; insert only rows missing from bronze.districts_dim."""
...
def _nominatim_lookup(display_name: str, *, delay_s: float) -> tuple[float, float] | None:
    time.sleep(delay_s)
    params = {
        "q": _nominatim_query(display_name),
        "format": "json",
        "limit": 1,
    }
